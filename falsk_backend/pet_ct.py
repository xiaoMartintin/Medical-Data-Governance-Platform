...
def ocr_pet_ct(pdf_path):
    ...
    if len(target_texts) == len(text_coords):
        ...
        header_patterns = {
            ...
            "gender": r"性别(.*?)年龄",
            "age": r"年龄(.*?)病历号",
            "case_history_number": r"病历号(.*?)申请科室",
            "department": r"申请科室(.*?)病案号",
            "medical_record_number": r"病案号(.*?)床号",
            "bed_number": r"床号(.*?)检查项目",
            "inspection_item": r"检查项目(.*?)显像剂",
            "photographic_developer": r"显像剂(.*?)剂量",
            "dosage": r"剂量(.*?)临床",
            "verdict": r"临床诊断(.*?)检查号",
            "exam_number": r"检查号(.*?)报告医生",
            "report_doctor": r"报告医生(.*?)审核医生",
            "review_doctor": r"审核医生签字(.*?)报告日期",
            "report_date": r"报告日期(.*?)报告",
        }
        ...
        structured_data["type"] = "核医学科PET-CT检查报告"
        pet_info = structured_data
        return pet_info
