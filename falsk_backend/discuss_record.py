...
def ocr_discuss_record(pdf_path):
    ...
    if len(target_texts) == len(text_coords):
        ...
        structured_data["type"] = "术前病例讨论记录"
        structured_data["charge_time"] = re.search(r"入院日期:(.*?)临床诊断", main_text).group(1).strip()
        structured_data["verdict"] = re.search(r"临床诊断:(.*?)手术指征", main_text).group(1).strip()
        structured_data["operation_trait"] = re.search(r"手术指征:(.*?)手术方式", main_text).group(1).strip()
        structured_data["operation_way"] = re.search(r"手术方式:(.*?)替代方案", main_text).group(1).strip()
        structured_data["replace_way"] = re.search(r"替代方案:(.*?)麻醉", main_text).group(1).strip()
        structured_data["anesthesia_way"] = re.search(r"麻醉方式:(.*?)讨论日期", main_text).group(1).strip()
        structured_data["discuss_date"] = re.search(r"讨论日期:(.*?)主持人", main_text).group(1).strip()
