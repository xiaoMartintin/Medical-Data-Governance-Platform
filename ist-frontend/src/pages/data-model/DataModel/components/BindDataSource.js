...
const BindDataSource = ({visible, onHideModal, model}) => {
    ...
    const fetchFields = (dataSource) => {
        ...
        switch (dataSource.dataSourceId) {
            case 1:
                setDataSourceFields(mockData.diseaseFieldDetail?.map(item => ({name: item.name, type: item.type})));
                break;
            case 2:
                setDataSourceFields(mockData.studyFieldDetail?.map(item => ({name: item.name, type: item.type})));
                break;
            case 3:
