...
import {collectModelData, deleteDataModel} from '../../../apis/data-model'
...
const DataModelCard = ({model, onBindModel, onFinishDelete, showOperation = true}) => {
    ...
    const handleViewDetails = (id) => {
        navigate(`/data-aggregation/model/${id}`)
    }
    ...
    return (<Card
        ...
        actions={[<Button onClick={() => handleViewDetails(model.id)}>详情</Button>,
        ...
        <p>标签: {model.tag.length > 0 ? model.tag.map((tag, idx) => (<Tag color='blue' key={idx} style={{
            margin: '2px 5px'
        }}>{tag}</Tag>)) : <Tag style={{margin: '2px 5px'}}>无</Tag>}</p>
